...
const getWeatherUser = async () => {
    ...
    let resp = await fetch(
        `${URL_WEATHER_API}lat=${latitude}&lon=${longitude}&appid=aae095cd0c99a0f95b09395b2c368f19`
    );
...
const createHabitats = async () => {
    ...
    const { icon, main: title } = weather[0];
    ...
    <div class="card">
        ...
        <div  class="card_content_description">
          ...
          <p class="content_description_text">${Math.round(main.temp - 273.15)}&deg</p>
        </div>
    </div>`;
